...
const MODULE_PATH: &[&str] = &["meta_signal_orchestrate", "authority"];
...
fn main() {
    ...
    .expect("assemble authority-approved privileged orchestration Interface transaction");
    let rust = rust_logos();
    ...
    BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
        .generate()
        .expect(
            "project privileged orchestration authority Interface from the verified transaction",
        )
        .write_or_check("META_SIGNAL_ORCHESTRATE_UPDATE_INTERFACE_ARTIFACTS")
        .expect("checked privileged orchestration authority Interface projections are fresh");
    CargoEthosSourceMetadata::new("meta-signal-orchestrate")
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("privileged orchestration authority bootstrap catalog is complete")
}
